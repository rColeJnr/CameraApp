...
import com.rick.cameraapp.databinding.FragmentPhotoFilterBinding
...
class PhotoFilterFragment : Fragment() {
    ...
    private fun getBitmapFromView(selectedImage: View): Bitmap? {
        val bitmap =
            Bitmap.createBitmap(selectedImage.width, selectedImage.height, Bitmap.Config.ARGB_8888)
    ...
    private fun applyFilter(filter: String) {
        when (filter) {
            ...
            "Swirl" -> {
                loadImage(SwirlFilterTransformation(0.5f, 1.0f, PointF(0.5f, 0.5f)))
            }
            "Invert filter" -> {
                loadImage(InvertFilterTransformation())
            }
            "Kuwahara filter" -> {
                loadImage(KuwaharaFilterTransformation(25))
            }
            "Sketch filter" -> {
                loadImage(SketchFilterTransformation())
            }
            "Toon filter" -> {
                loadImage(ToonFilterTransformation())
            }
        }
    }
